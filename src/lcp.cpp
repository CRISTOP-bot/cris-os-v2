...
static const size_t MAX_PACKAGES = 16;
static const size_t MAX_DEPENDENCIES = 8;
static const size_t MAX_FILES = 8;
static const size_t REPO_BUFFER_SIZE = 8192;
...
static void lcp_split_list(const char* text, const char** out, size_t* count, size_t max_count) {
    *count = 0;
    const char* p = text;
...
static void lcp_parse_package_block(char* block) {
    if (package_count >= MAX_PACKAGES) return;
    LcpPackage* pkg = &packages[package_count];
    pkg->name = 0;
    pkg->version = 0;
    pkg->description = 0;
    pkg->arch = 0;
    ...
    while (line && *line) {
        ...
        if (lcp_strncmp(line, "name:", 5) == 0) {
            ...
            lcp_split_list(value, pkg->dependencies, &pkg->dependency_count, MAX_DEPENDENCIES);
        } else if (lcp_strncmp(line, "files:", 6) == 0) {
            value = lcp_trim(line + 6);
            lcp_split_list(value, pkg->files, &pkg->file_count, MAX_FILES);
...
static bool lcp_parse_repo(const char* data, size_t size) {
    if (size >= REPO_BUFFER_SIZE) return false;
...
"name:libc\n"
"version:1.0.0\n"
"description:Biblioteca estándar mínima para CrisOS\n"
"arch:i386\n"
"maintainer:CrisOS Team\n"
"license:MIT\n"
"dependencies:\n"
...
bool lcp_init() {
    const FSFile* file = fs_find("lcp_repo.txt");
    if (file) {
...
static LcpPackage* lcp_find_package(const char* name) {
...
static void lcp_print_package_info(const LcpPackage* pkg) {
    ...
    if (pkg->dependency_count == 0) {
        ...
        for (size_t i = 0; i < pkg->dependency_count && op + 1 < sizeof(output); ++i) {
            ...
            const char* s = sep;
            while (*s && op + 1 < sizeof(output)) output[op++] = *s++;
            s = dep;
            while (*s && op + 1 < sizeof(output)) output[op++] = *s++;
            sep = ", ";
        }
...
static void lcp_list_installed() {
    for (size_t i = 0; i < package_count; ++i) {
...
static bool lcp_has_dependents(const char* name) {
    for (size_t i = 0; i < package_count; ++i) {
        ...
        for (size_t j = 0; j < packages[i].dependency_count; ++j) {
            if (lcp_strcmp(packages[i].dependencies[j], name) == 0) return true;
        }
    }
    return false;
}

static void lcp_install_package(LcpPackage* pkg, bool no_deps) {
    if (pkg->installed) {
    ...
    if (!no_deps) {
        for (size_t i = 0; i < pkg->dependency_count; ++i) {
            LcpPackage* dep = lcp_find_package(pkg->dependencies[i]);
...
static void lcp_remove_package(LcpPackage* pkg, bool purge) {
    ...
    pkg->installed = false;
    console_print("Removed "); console_print(pkg->name);
    if (purge) {
...
static void lcp_show_dependencies(LcpPackage* pkg) {
    if (!pkg) {
        console_print("Package not found.");
        return;
    }
...
static void lcp_verify_package(LcpPackage* pkg) {
    if (!pkg) {
        console_print("Package not found.");
        return;
    }
...
int lcp_handle_command(const char* args) {
    ...
    if (lcp_strcmp(token, "info") == 0) {
        ...
        LcpPackage* pkg = lcp_find_package(rest);
        if (!pkg) {
    ...
    if (lcp_strcmp(token, "install") == 0) {
        ...
        LcpPackage* pkg = lcp_find_package(name);
        if (!pkg) {
            lcp_print("Package not found.");
            return 0;
        }
        if (pkg->installed && !force) {
