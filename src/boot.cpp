...
static const size_t MAX_BOOT_UNITS = 16;
static const size_t MAX_BOOT_NAME = 32;
static const size_t MAX_BOOT_DESC = 64;
static const size_t MAX_BOOT_CMD = 128;
...
static BootUnit units[MAX_BOOT_UNITS];
...
static void bt_parse_unit(const FSFile* file) {
    if (!file || unit_count >= MAX_BOOT_UNITS) return;
    const char* name = bt_get_basename(file->name);
    const char* ext = name;
    while (*ext && *ext != '.') ++ext;
    if (!bt_starts_with(ext, ".boot")) return;

    BootUnit* unit = &units[unit_count];
    unit->name[0] = '\0';
...
static void bt_print_unit(const BootUnit* u) {
    bt_print(u->name);
    bt_print(" [");
...
static BootUnit* bt_find_unit(const char* name) {
    ...
    for (size_t i = 0; i < unit_count; ++i) {
        if (bt_strncmp(units[i].name, name, name_len) == 0 && units[i].name[name_len] == '\0') {
            return &units[i];
        }
    }
    return 0;
...
static void bt_start_unit(BootUnit* unit) {
    if (!unit) return;
    if (unit->active) {
...
static void bt_stop_unit(BootUnit* unit) {
    if (!unit) return;
    if (!unit->active) {
...
static void bt_start_default_target() {
    bool started = false;
    for (size_t i = 0; i < unit_count; ++i) {
...
int boot_handle_command(const char* args) {
    ...
    if (bt_streq(command, "status")) {
        ...
        BootUnit* unit = bt_find_unit(target);
        if (!unit) {
