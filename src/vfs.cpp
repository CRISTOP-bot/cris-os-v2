...
static char* vfs_alloc_data(const char* data, size_t size) {
    if (data_used + size >= VFS_DATA_STORE_SIZE) return 0;
...
static int vfs_find_child(int dir, const char* name) {
    if (dir < 0 || dir >= VFS_MAX_NODES || !nodes[dir].is_dir) return -1;
...
static int vfs_add_child(int parent, int child) {
    if (parent < 0 || parent >= VFS_MAX_NODES) return -1;
    if (nodes[parent].child_count >= VFS_CHILDREN_MAX) return -1;
...
static int vfs_resolve(const char* path) {
    ...
    if (!path || path[0] == '\0') return current;
    if (path[0] == '/') {
        current = root_node;
        ++path;
    }
...
static int vfs_create_node(const char* path, bool dir) {
    ...
    if (!nodes[parent].is_dir) return -1;
    ...
    if (vfs_add_child(parent, node) < 0) return -1;
    return node;
}
            ...
            if (child < 0) {
                ...
                if (*p != '\0') {
                    ...
                    if (node >= 0) {
                        ...
                        nodes[node].child_count = 0;
                        if (vfs_add_child(current, node) < 0) continue;
                    }
                } else {
                    node = vfs_new_node();
                    if (node >= 0) {
...
bool vfs_cd(const char* path) {
    ...
    if (node < 0 || !nodes[node].is_dir) return false;
    cwd_node = node;
    return true;
}
...
static bool vfs_remove_node(int node) {
    if (node < 0 || nodes[node].read_only) return false;
    if (nodes[node].is_dir && nodes[node].child_count > 0) return false;
    int parent = nodes[node].parent;
    if (parent >= 0) {
        int write = 0;
        for (int i = 0; i < nodes[parent].child_count; ++i) {
            if (nodes[parent].children[i] == node) continue;
...
bool vfs_cp(const char* src, const char* dst) {
    ...
    int parent;
    char name[VFS_NAME_SIZE];
    parent = vfs_resolve_parent(dst, name);
    if (parent < 0 || !nodes[parent].is_dir) return false;
...
bool vfs_mv(const char* src, const char* dst) {
    ...
    if (old_parent >= 0) {
        ...
        for (int i = 0; i < nodes[old_parent].child_count; ++i) {
            if (nodes[old_parent].children[i] == node) continue;
...
bool vfs_write(const char* path, const char* content, size_t length) {
    ...
    if (nodes[node].is_dir) return false;
